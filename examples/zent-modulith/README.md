...
Demonstrates **ZigModu** (Application / HTTP / **ComptimeRouter**) with **[zent](https://github.com/chy3xyz/zent)** v0.74.2 as the schema-as-code data layer (demos span v0.21–v0.74 capabilities).
...
`build.zig.zon` pins the released tag `v0.74.2`; for local development against a
...
# ZENT_DEV_TOKEN=1 exposes the demo token mint (POST /api/v1/dev/token).
# It mints a token for any tenant_id — never enable it in production.
ZENT_DEV_TOKEN=1 HTTP_PORT=18100 zig build run
```
...
TOKEN=$(curl -s -X POST 'http://127.0.0.1:18100/api/v1/dev/token?tenant_id=1&sub=100' \
  | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')

# 2) products routes are JWT-protected and tenant-scoped from the token's
#    `aud` claim — a `?tenant_id=` override in the URL is ignored.
...
curl -s -H "Authorization: Bearer $TOKEN" -X POST 'http://127.0.0.1:18100/api/v1/products' \
  -H 'Content-Type: application/json' -d '{"name":"Widget","price_cents":1999,"price":"19.99"}'
curl -s -H "Authorization: Bearer $TOKEN" \
  'http://127.0.0.1:18100/api/v1/products?page=1&page_size=20'
curl -s -H "Authorization: Bearer $TOKEN" 'http://127.0.0.1:18100/api/v1/products/1'
```
...
`.attr`（JWT 中间件写 `tenant_id`，**本演示用这个**）或 `.query`（会信任客户端传的 tenant——只允许在公开 demo 里用）。

```bash
# list（paged 信封 {code, items, total}；page/page_size 自动钳制）
curl -s -H "Authorization: Bearer $TOKEN" 'http://127.0.0.1:18100/api/v1/products?page_size=10000'
# create（JSON body；tenant 取自 JWT 的 aud claim → attr，不入 body）
curl -s -H "Authorization: Bearer $TOKEN" -X POST 'http://127.0.0.1:18100/api/v1/products' \
  -H 'Content-Type: application/json' -d '{"name":"Widget","price_cents":1999,"price":"19.99"}'
# get / update / delete（跨租户访问返回 404）
curl -s -H "Authorization: Bearer $TOKEN" 'http://127.0.0.1:18100/api/v1/products/1'
curl -s -H "Authorization: Bearer $TOKEN" -X PUT 'http://127.0.0.1:18100/api/v1/products/1' \
  -H 'Content-Type: application/json' -d '{"name":"Widget Pro","price_cents":2199,"price":"21.99"}'
curl -s -H "Authorization: Bearer $TOKEN" -X DELETE 'http://127.0.0.1:18100/api/v1/products/1'
```
...
```bash
# 注意：本段是 data-scope 机制的公开演示，user_id/tenant_id/scope 均来自
# query 以便手工调试；生产环境必须像 products 一样从 JWT claim 派生。
...
- **paged()**：泛型 list 即 `GET /api/v1/products?page=1&page_size=2`
  → `{"code":0,"items":[...],"total":N}`（内建 count + limit/offset，统一释放）。
- **CountBy()**：`GET /api/v1/products/counts` → 每租户行数（单条 GROUP BY）。
- **ContainsEscaped()**：`GET /api/v1/products/search?q=100%25`（public 自定义端点，tenant 走 query）
