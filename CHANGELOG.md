...
### 第 45 批（诊断，第二步）：夜间 `Fuzz` 的 `corrupted coverage file` 是**次生症状** —— 某个 test 二进制静默非零退出，覆盖文件被截断（头部声明 6025 个 PC、文件只有 24 字节），而合并的报错**顶替**了它真正的失败信息

第 41/43 批已排除"缓存陈旧"与"runner 文件系统"。本批**拿到产物的字节**（转储生效）：

```
--- .zig-cache/v/59e4bde28e630904 (24 bytes)
 89 17 00 00 00 00 00 00 00 00 00 00 00 00 00 00
 00 00 00 00 00 00 00 00
```

`89 17` = 6025（小端 u64 的头部字段），后面全零 —— 也就是**头部声明了 6025 个 PC，然后是截断**。
配上同一段日志里的 `failed command: …/o/9d1d363de09ba3c108584c4fdf369565/test --seed=0x8ab51111 --listen=-`：
**一个 test 二进制非零退出、而且一行输出都没有**，它留下的覆盖文件就是那个 24 字节的残片；
构建系统随后在合并时死在这个残片上，报出 `pcs_len was zero` —— **把真正的失败原因顶掉了**。
（本机同一条命令绿，所以这也是"只在那台 runner 上"的形状。）

**本批的改动（诊断，不是修复）**：给 fuzz 命令加 `--summary all`，让下一次失败的摘要**按名字**指出是哪一个
step／二进制；保留失败时的产物字节转储。**下一步**：拿到那个名字之后，直接单独跑它（小 bound + 显式捕获退出码），
把"静默非零退出"的现场换成可读的失败 —— 覆盖文件的报错只是它的影子。
