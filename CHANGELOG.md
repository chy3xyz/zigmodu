...
> **后续更正（第 49 批已定案）**：本节有两处判断是**错的**，不要照它读 ——
> ① "某个 test 二进制**静默非零退出**、一行输出都没有"：那两行 `failed command:` 是 Zig 构建系统的**装饰性输出**
> （任何往 stderr 写过字的 run step，**包括成功的**，都会被补打一行；本机一次绿运行里就有）；
> ② "覆盖文件是**半写/截断**、头部声明了 6025 个 PC"：24 字节是**完整头部**，offset 0 的 6025 是 `n_runs`、不是 PC 数。
> 真正的根因见第 49 批（x86_64 默认后端不发 `--fuzz` 覆盖 section）。
> 下表的"排除"结论仍然成立（那些候选确实不是原因）。

**这个步骤自 2026-09-25 第一次真正执行起，每一次都红。** 四轮实验排除了所有"明显"的候选：

| 候选 | 实验 | 结果 |
|---|---|---|
| 缓存里的陈旧产物 | 先 `rm -rf .zig-cache/v` 再跑 | 照样红、同一哈希 ⇒ **排除** |
...
`failed command`（没有任何其它文字），留下的产物是**半写**的（头部声明 6025 个 PC，文件 24 字节就断），
随后合并报的 `corrupted coverage file …: pcs_len was zero` 是**次生症状**、把进程真正死因顶掉了。
同一条命令在本机绿（一次绿运行只留一个 1.25 MB 的合并产物），push 流水线在同一台 Linux runner 上的
`zig build test` 也是绿的 —— 差别就在 `--fuzz` 走的覆盖那条路。

**本批的处理是可见性决定，不是修复**：给这一步加 `continue-on-error: true`，并在注释里写清上面这张表与
"原因未知"。理由：一个每次都红的步骤会让**整个夜间永远红**，从而掩盖它上面那三步将来真的失败 —— 那正是
本次刚刚在 push 腿修掉的那种掩盖（把 `soak-smoke` 挪到 `Run tests` 之后）。**并且要说准还剩什么可见**：
加了它之后，这一步在 run summary 里的**结论是 `success`**，所以活下来的证据是它的**日志**（`failed command`
行、`--summary all` 的输出、以及失败时的产物转储），不是步骤列表里的一抹红。原因查清后删掉这个 flag。
（夜间 job 因此在 2026-09-25 第一次**四步全绿**：soak / cluster soak / runtime stress / fuzz。）
...
剩下两个候选，本批把 fuzz 步改成"一次运行就能区分它们是哪一个"：

1. **runner 的文件系统把覆盖写截断** → 第 43 批把 `ZIG_LOCAL_CACHE_DIR` 指到 `${{ runner.temp }}` 下
   （本地缓存连同覆盖目录一起换文件系统）。**失败跟着目录走 ⇒ 是 (1)**。
2. **fuzz 模式在 runner 上的子进程/归并形状留下空产物** → 若换了目录仍以同一方式失败，就落在这一条。

另外给这一步加了失败时的证据转储：两个位置各自的 `v/` 列表 + **每个产物的字节数与头 32 字节** ——
即便这一跑是绿的，也会留下"到底写出了什么"的记录。**本批结束后按答案把这两处临时改动收掉。**
...
2026-09-25 第一次真正被执行（第 6 步 `Cluster soak` 过去几天一直卡死，第 7/8 步从来没轮到过）。
它的红不是用例红了，而是构建系统收尾时拒绝一个覆盖文件：

```
error: step run test: corrupted coverage file .zig-cache/v/59e4bde28e630904: pcs_len was zero
```

**我做的修法（已进 CI）**：夜间 job 在 `Install deps` 之后加一步 `rm -rf .zig-cache/v`。

**实验把它否掉了 —— 这条要留着，别让下一个人照着我的错误假设再修一遍**：那一步**在位**的情况下，
下一次 dispatch 的 fuzz 步**以相同方式失败、而且是同一个哈希**（`59e4bde28e630904`，13:59:53）。
所以那个覆盖文件是**这次运行自己写出来的**，构建系统在归并时拒绝它；**不是**缓存里的陈旧产物。
（本机同一条命令 `exit=0` 全绿，所以也不是"用例红了"。）

**该步骤仍然保留**，但它现在的注释与 CHANGELOG 都写明"这是卫生措施、不是修复"：覆盖文件是机器状态、
跨运行不可复用，构建前删掉它本身无害且正确，而 workflow 里没有任何步骤消费它。

**剩下两个候选（下一次实验要能区分它们）**：
1. **runner 的文件系统/写入**：覆盖文件被截断成 0 PC（overlay 上的写没落全）。区分实验：把那一步的
   `ZIG_LOCAL_CACHE_DIR` 指到 `$RUNNER_TEMP` 下的新目录（同一台 runner、不同文件系统），看是否还红。
2. **fuzz 模式下 runner 的子进程/覆盖归并**：只有那个 runner 会走到的形状（例如某条只在 CI 上跑的
   门控用例在 fuzz 模式下留下 0 PC 的产物）。区分实验：`--fuzz=2000` 之外再跑一次不带 `--fuzz` 的
   `zig build test`（同一 runner），若只有 fuzz 红，就落在归并/子进程这条线上。
...
**`src/runtime/delivery_log.zig`（新增，1805 行 / 12 条测试）—— 投递轨分级存储的"盘上那一半"。**
`docs/RUNTIME.md` §13.3 Q4 早就把"落盘"列为第二档并指出前提是 **codec 契约**。这次只做**存储层**：
分段日志（`delivery-<20 位 id>.log`）、**逐字节格式在文件头写明**（magic `"ZDL1"` + 显式 `format_version`
+ 全部字段显式小端 + 每帧 `CRC-32/ISCSI`）、**撕裂尾部与载荷损坏是两个不同的结论**（前者可容忍、给出字节数、
`repair()` 显式截断且拒绝被损坏的记录；后者拒收并给出段/偏移/下标；**不做 resync** —— resync 必须信一个
已经不可信的长度字段）、分段轮转**绝不拆帧**（超限记录独占一段）、`append` **零分配**（`Writer` 里没有
allocator 字段，32 字节栈上帧头 + 三次 positional 写；测试用 `FailingAllocator` 把 `fail_index` 钉在
当前位置，任何一次分配都会让用例炸而不是只是变慢）。

**为什么不是复用 `src/core/eventbus/WAL.zig`（决定与理由都写在文件头）：** 它是**已发布的磁盘格式**，
不是内部件 —— `root.zig` 导出 `WAL`/`WALConfig`，`DistributedEventBus`/`SagaOrchestrator`/`ai/workflow`
都用**调用方给的 `dir_path`** 构造它，文件落在部署方自己选的位置。复用 = 原地改格式 = 让磁盘上已有的文件
不可读，而"没有生产文件依赖它"这个前提**拿不到证据**（恰好相反）。逐条缺口（无 magic、无版本号、
端序靠 `@bitCast` 的隐式主机端序、无任何校验和、无 record kind、撕裂尾部静默 `break`、每条记录一次
`ArrayList`）列在文件头的表里。**两个格式两个名字是诚实的结果；给已经在磁盘上的文件补一个 magic 不是。**

**范围诚实（别读成"已经能落盘重放了"）**：它**没有**从 `runtime.zig`/`root.zig` 导出（还没有消费者），
也**没有**接 recorder —— 把一条投递记录变成字节的 codec 契约、以及 `TrackRef.payload_codec` 那个预留位
**仍未做**。所以 §13.5 的边界没变，`docs/RUNTIME.md` §13.3 Q4 已按这个口径改写：**"盘上那一半有了，
进盘那一半没有"**。

> **红证据（11 组守卫逐个改弱跑红，逐条给出那一行原始输出）**：撕裂尾部逐字节偏移（改回静默 `break`
> → `expected 1, found 0`）、跨段拆帧（帧头写旧段/帧体写新段 → `expected 100, found 132`，另一条读侧
> `FAIL (TornTail)`）、翻字节（CRC 只覆盖帧头 → 载荷被当数据交付）、错误 magic / 错误版本各自两个错误、
> `append` 零分配（给 `Writer` 加 allocator 并在 append 里 dupe → 第一次 append 就 `OutOfMemory`）、
> `repair` 的幂等报告面与"拒绝被损坏的记录"、重开时必须拒绝损坏尾部（`error.LogDamaged`）、
> `seq` 必须严格递增。
> **一条没拿到红证据并写明了**：`repair` 幂等的**行为面**（第二次 `repair` 在干净日志上本来就是 no-op，
> 尺寸不变、测试区分不出的只有报告）—— 不当成有效红证据。

**`docs/RUNTIME.md` §12.16：公平性与 `batch` 的实测（6 个点 × 3 种形状 × 每点 27 轮）。**
设计里"每个 worker 至多一个 ready token + FIFO 环 + 一次 claim 跑一个 batch"是否真的不饿死别人，
此前只是构造性论证。量法是：1 条池线程、A 被持续灌到邮箱永不空、B/C 各一条一条地来（各 3000 次探针）、
`batch` 取默认 16。

* **被断言的是"探针等待期间 A 跑过多少条消息"**（宿主无关的那一半）：10 次运行 × 2 条通道 × 3000 次探针，
  max **每次恰好 16**，而 A 在同一区间跑了约 **9.6 万条**。界取 `4 × batch`，理由（最坏形状是两个 batch、
  参考点取在 `send` 之后所以读数是下限、比"无界"小三到四个数量级）都写在节里。
  **`batch` 就是公平本身** —— 临时把它改成 65536，探针就等 53118 条、断言红（那条原始输出在节里）。
* **重载那次是最有用的一条**：同机另有编译时，生产者的重试次数从 ~9 万涨到 **676 040**、B 的 max 从
  22 µs 变 **5 183 µs**（时钟上 236 倍），而**表 1 的 max 仍然是 16** —— 这就是把界钉在消息数上的意义
  （与 §12.15 的 `hostCanHoldTheBound` 同一思路：能跨宿主的那条拿去断言，宿主相关的那条打印）。
* **尾巴不是 `batch`，是 park**：三张表里每个点每个形状的 p99/max 都是 ~1 ms，因为池的 park 是
  `idle_wait_ms = 1` 的轮询（`scheduler.zig:201`）；`batch` 从 1 调到 64，p99 一点没动。
  想压池化 worker 的 p99，要看的是那条取舍。
* **`batch` 结论：保持 16。** 1 是唯一被数据否掉的默认值（`mixed-busy` p50 最高 146 µs、吞吐只有 16 的
  46%）；8/16/32/64 落在同一片平台（32 只比 16 高 5%），而单次运行的 9 个中位数散布是 1.2×–2.4×，
  点间差别全在噪声里 —— 平台既平就取平台上最小的一点，因为 `batch` 是**公平上界**，加倍它买到的是确定
  的代价与测不出的收益。**唯一反例写清了**：`mixed-busy` 里 32 比 16 高 15%，但两者区间重叠、切不开。
* 节里同时写明**没说明什么**：时间列是同机有其他编译时的读数（跨宿主引用前先重跑）、没测重 handler 形状、
  没测 `.blocking` 池、生产者是自旋重试（换成 `sendBlocking` 会同时改变吞吐与 park 频率）。
* §12 里三处**已过时**的"未做 `batch` 调优"（§12 开头、§12.9 第 1 条、§12.12 末尾）按结果改写，
  设计原文其余部分原样保留为决策记录。
