...
## Version
- Framework: **v0.15.45** (`build.zig.zon`)
...
- Package **v0.15.45** · Zig **0.17.0** · GitHub `chy3xyz/zigmodu` · branch `master`.
...
- zent v0.41.1 与 `data.sqlx` 正交，勿混驱动/共享事务（`docs/ZENT.md`）；`examples/zent-modulith` 按 v0.41.1 能力演示（v0.13 起向后兼容）。v0.32.3 起 sqlite 单连接访问串行化（`Rows` 持锁至 `deinit()`）；v0.33.0 起 `UseInterceptor` 覆盖 Create/BulkInsert（create 上 `whereEq` = 缺省才填）；v0.35.0 起 outbox 认领式派发（崩溃遗留用 `requeueStale` 回收）；v0.36.0 起迁移默认加锁、outbox 新增 `claimed_at` 列、`createAllTables` 增加 allocator 参数；v0.37.0 起 `max_wait_ms` 真正阻塞等待、嵌套预加载每层一次查询；v0.38.0 起 `queryTargets*` fail-closed（旧语义改名 `*Unscoped`），新增 NULL 容忍扫描器；v0.39.0 起 `zent.scope` 让手写 SQL 也能带上软删/隐私/拦截器契约（**裸 SQL 不再自动隔离，必须接 scope**），并有 `<col>Like` 与 `zent.version`；v0.40.0 起一行式释放 `deinitRows`/`deinitRow`/`deinitEdgeRows`（示例已全部改用，整页需 `var rows`），v0.41.0 明确 `crud_helpers.queryRows` 也是需要 `zent.scope` 的裸路径，v0.41.1 修复 `deinitRows` 指针形态回归。升级依赖后**先删 `.zig-cache` 重建**（增量缓存可能沿用旧 fetch 模块，本次适配实测踩中）。
