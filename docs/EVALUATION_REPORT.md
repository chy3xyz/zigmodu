...
| 9 | **测试质量** | 97 | 93 | +4 | 990/1009 passed；integration 含 JWT + CRUD + shopdemo smoke |
...
| 5 | OTLP/Vault HTTPS | 中 | ✅ `http(s)://` 均通（`std.http.Client` TLS 1.3，系统 CA） |

---

## 结论

ZigModu **v0.15.34** 在 Zig 0.17 上约 **98/100**。短请求本机场景优先 HTTP/1.1；h2c/gRPC 适合多路复用与流式。TLS 生产路径用 sidecar ALPN，OTLP/Vault 出站 HTTPS 经系统 CA。
