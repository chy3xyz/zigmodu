...
**延迟分布表**：runtime 组 9 条 + 两条链（`App lifecycle`、`1L x10M events`），每行 `p50 / p95 / p99 / p99.9`
（ns/op），前缀是 `[pct]`，与 `[med3]` 同为“原样打给日志看”的输出。
...
- **`sum = N x med3` 这一列要读。** 每行末尾把该指标 1000 个 batch 的**自计时之和**与它的 `[med3]` 值并列
  ...
  `Worker spawn+join` 落在 0.84–1.23×）说明 batch 化没有改动这条指标；差出 0.8–1.25 的区间时，
  行下会多一段说明 —— 已知成因有两类，那段说明分不出来是哪一类：**batch 化真的改了工作负载**（`TimerWheel
  x100K` 每 batch 只有 100 个定时器节点、缓存全热，而 `[med3]` 那一轮持有 10 万节点，本机实测 0.34–0.49×），
  和**宿主负载在 `[med3]` 阶段与分位数阶段之间变了**（本机在一次编译同时进行时，`HotBus 8sub x1M`、
  `Mailbox full-path x10M` 实测 1.37–1.57×，而它们各自的 `[med3]` 也一起被拉高了）。两种情况的分位数都
  只描述采样时看到的那份形状。
- **`alloc/op` 没有做，是有意的。** 统计分配要在被计时路径上插一个计数 allocator，那会改动被测路径本身
  （每次分配多一层间接与计数写入）；要做也得是另一份专门的 instrumented harness，而不是挂在延迟采样上。
