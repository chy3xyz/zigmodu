...
`TimerWheel x100K` 是**唯一被排除**的一条 runtime 指标（它的 `[med3]` 照旧测量、照旧判定），理由与代价见
本节末尾那条；表头也会打印这行排除说明，所以在日志里 grep 不到它时先看那两行。每节末尾另外打印自己的
...
- **`TimerWheel x100K` 为什么不在表里。** 因为它的**每轮代价是活结构的规模**，不是每轮代码：一轮的活是遍历
  ...
  `schedule` 那一半的差在 `nodes` 哈希表（10 万条 → 桶数组 ~3.5 MB，每次 put/remove 都是缓存缺失）与节点
  内存的首次触碰，`advance` 那一半的差在它要走的那 5.6 MB 节点。于是 sum 常年落在 `[med3]` 的 0.34–0.51×
...
- **`alloc/op` 是另一趟 instrumented 分测，不在 `[pct]` 里。** 统计分配要在被计时路径上插一个计数 allocator，
  ...
  `Wheel.cancel`·`advance` 都是 0，`Wheel.schedule` 是**每个定时器一次节点分配**（实测 100,015 次 / 10 万个
  定时器 = 1.00 alloc/op，多出的 15 次是 `nodes` 哈希表的摊还扩容，正是那份测试自己注释掉的“首次插入还有一次
  桶数组分配”）；`Worker spawn+join` 实测 1.01 alloc/op（1000 次 spawn 共 1011 次分配，其中**stop/join 那半边
