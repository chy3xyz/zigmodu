...
写这份文档时的实测（`src/test/AiBoundary.zig` 会持续核对）：

| 方向 | 现状 |
|------|------|
| **框架 → ai** | **干净**：整个 `src/` 里只有 `root.zig` 的一行 re-export（`pub const ai = @import("ai/ai.zig")`） |
| **ai → 框架** | 直接 `sqlx/sqlx.zig` **30** 处、直接 `persistence/backends/**` **26** 处，分布在 21 个文件里 |
...
`src/test/AiBoundary.zig` 里的 `baseline_direct_sqlx` / `baseline_direct_backends` 就是那两个上限；
搬完一个文件就把数字减一。**抽包条件**：两者都到 0，且 `AI boundary` 两个测试持续为绿。
