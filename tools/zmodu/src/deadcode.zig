...
pub fn run(io: std.Io, allocator: std.mem.Allocator, args: []const []const u8) u8 {
    ...
    for (args) |arg| {
        if (!applyFlag(&cli, arg)) {
            cli.paths.append(allocator, arg) catch return fail("out of memory", .{});
        }
    }
    ...
    const files = scanner.scan(allocator, io, cwd, cli.paths.items, .{
    ...
    for (files) |f| {
        const content = std.Io.Dir.cwd().readFileAlloc(io, f.path, allocator, .unlimited) catch |err| {
            std.debug.print("zmodu deadcode: cannot read {s}: {s}\n", .{ f.display, @errorName(err) });
            continue;
        };
        const source: [:0]u8 = allocator.allocSentinel(u8, content.len, 0) catch |err| {
            return fail("out of memory: {s}", .{@errorName(err)});
        };
        @memcpy(source[0..content.len], content);
        source[content.len] = 0;
        sources.append(allocator, .{ .path = f.path, .display_path = f.display, .source = source }) catch |err| {
            return fail("out of memory: {s}", .{@errorName(err)});
        };
    }
    ...
    for (cli.paths.items) |p| {
        ...
        root_paths.append(allocator, abs) catch |err| {
            return fail("out of memory: {s}", .{@errorName(err)});
        };
    }

    var result = analyze.analyze(allocator, sources.items, .{
    ...
    const output = if (cli.json)
        report.formatJson(allocator, &result, sources.items) catch |err| {
            return fail("formatting failed: {s}", .{@errorName(err)});
        }
    else
        report.formatHuman(allocator, &result, sources.items, .{
            .json = false,
            .verbose = cli.verbose,
        }) catch |err| {
            return fail("formatting failed: {s}", .{@errorName(err)});
