...
fn emitService(allocator: std.mem.Allocator, e: *const Entity, P: []const u8) ![]const u8 {
    var buf = std.ArrayList(u8).empty;
    errdefer buf.deinit(allocator);
    try appendPrint(allocator, &buf, "//! Generated by zmodu saas — {s} service\n", .{e.name});
    try buf.appendSlice(allocator,
        \\const std = @import("std");
        \\const model = @import("model.zig");
    ...
    try appendPrint(allocator, &buf, "    pub fn list(self: *@This(), allocator: std.mem.Allocator, org_id: i64, page: usize, size: usize) !std.ArrayList(model.{s}) {{\n", .{P});
    try buf.appendSlice(allocator, "        return self.persistence.list(allocator, org_id, page, size);\n    }\n\n");
    try appendPrint(allocator, &buf, "    pub fn get(self: *@This(), allocator: std.mem.Allocator, org_id: i64, id: i64) !?model.{s} {{\n", .{P});
    try buf.appendSlice(allocator, "        return self.persistence.get(allocator, org_id, id);\n    }\n\n");
    try appendPrint(allocator, &buf, "    pub fn create(self: *@This(), e: model.{s}) !i64 {{\n", .{P});
    try buf.appendSlice(allocator, "        try self.validate(e);\n        return self.persistence.create(e);\n    }\n\n");
    try appendPrint(allocator, &buf, "    pub fn update(self: *@This(), e: model.{s}, org_id: i64) !void {{\n", .{P});
    try buf.appendSlice(allocator, "        try self.validate(e);\n        try self.persistence.update(e, org_id);\n    }\n\n");
    try appendPrint(allocator, &buf, "    pub fn delete(self: *@This(), org_id: i64, id: i64) !void {{\n", .{});
    try buf.appendSlice(allocator, "        try self.persistence.delete(org_id, id);\n    }\n\n");
    try appendPrint(allocator, &buf, "    pub fn validate(_: *@This(), e: model.{s}) !void {{\n", .{P});
    for (e.fields) |f| {
...
fn emitApi(allocator: std.mem.Allocator, e: *const Entity, P: []const u8) ![]const u8 {
    var buf = std.ArrayList(u8).empty;
    errdefer buf.deinit(allocator);
    try appendPrint(allocator, &buf, "//! Generated by zmodu saas — {s} API (ComptimeRouter, tenant-scoped, JWT)\n", .{e.name});
    try buf.appendSlice(allocator,
        \\const std = @import("std");
    ...
    try buf.appendSlice(allocator, " ");
    try buf.appendSlice(allocator, P);
    try buf.appendSlice(allocator, "Api = struct {\n    service: *service.");
    try buf.appendSlice(allocator, P);
    try buf.appendSlice(allocator,
        \\Service,
        \\
