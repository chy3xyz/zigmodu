//! Canonical home of the comptime struct-validation engine.
//!
//! `validateStruct` / `validateStructCollect` (plus `Violation`, `Violations`
//! and `MessageHook`), the scalar checkers (`notEmpty`, `email`, `uuid`, `url`,
//! `phone`, `range`, `oneOf`, `minLength`, `maxLength`, `Result`) and the
//! multi-check `Validator` struct all live here. `http.validateRequest`
//! (`api/middleware/Validation.zig`) and `http.extractJsonValidated`
//! (`api/Extract.zig`) read this file directly, so neither chain resolves
//! through the deprecated `validation/Validator.zig`.
//!
//! Why a *sibling* of `validation/FieldRules.zig` rather than an extension of
//! it, and why a new name rather than `Validator.zig`:
//!
//! * `FieldRules.zig` is the *declaration* of the rule shape, and its one hard
//!   property is that it imports nothing — that is what lets the http domain
//!   read the rule struct without dragging the engine (and `sqlx/errors.zig`)
//!   along, and what makes the two files a clean pair instead of a cycle. The
//!   engine needs both `std` and `../sqlx/errors.zig` (`Validator.validate`
//!   returns `errors.Result`), so it belongs in its own file.
//! * `Validator.zig` is the *deprecated* file's name: the whole point of the
//!   move is that the canonical implementation is no longer behind the
//!   deprecation banner and is no longer the thing `http.*` has to import.
//!
//! Dependency direction, pinned by `src/test/ApiFreeze.zig`: this file imports
//! `FieldRules.zig` and never `Validator.zig`; `Validator.zig` is the thin alias
//! layer that points back here.

const std = @import("std");
const errors = @import("../sqlx/errors.zig");
/// Field validation rules for comptime struct validation — the canonical
/// declaration lives in `validation/FieldRules.zig`; the engine re-exports it so
/// a caller can write `FieldValidation.FieldRules` without a second import.
